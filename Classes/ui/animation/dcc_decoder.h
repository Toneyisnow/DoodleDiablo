...
typedef struct
{
	...
	// Sprite * bmp;   // sub-bitmap in the buffer bitmap
	DsBitMapData * bmp;
...
typedef struct
{
	...
	int             nb_cells_w;
	int             nb_cells_h;
	/// Sprite          * bmp;        // frame buffer bitmap
	DsBitMapData     * bmp;
...
public:
	...
	DCC_S * dcc_disk_load(const char * dcc_name);
	...
	int     dcc_save_frame(DCC_S * dcc, int d, int f, DsBitMapData * frm_bmp);
	int     dcc_make_frames(DCC_S * dcc, int d);
	int     dcc_make_frames2(DCC_S * dcc, int d);
