...
int DccDecoder::dcc_read_bytes(DCC_BITSTREAM_S * bs, int bytes_number, void * dest)
{
	int i;
...
// return 0 on success, non-zero if error
int DccDecoder::dcc_read_bits(DCC_BITSTREAM_S * bs, int bits_number, int is_signed,
	UDWORD * dest)
{
...
// return 0 on success, non-zero if error
int DccDecoder::dcc_frame_header_bitstream(DCC_S *dcc, DCC_BITSTREAM_S *bs, int d, int f)
{
...
void DccDecoder::dcc_init_dir_bitstream(DCC_S * dcc, DCC_BITSTREAM_S * bs, int d)
{
...
// return 0 on success, non-zero if error
int DccDecoder::dcc_optional_datas(DCC_S * dcc, DCC_BITSTREAM_S * bs, int d)
{
	UDWORD size = 0;
	int    f;
	char   add_error[80];
...
// return 0 on success, non-zero if error
int DccDecoder::dcc_other_bitstream_size(DCC_S * dcc, DCC_BITSTREAM_S * bs, int d)
{
	char add_error[80];
...
// return 0 on success, non-zero if error
int DccDecoder::dcc_pixel_values_key(DCC_S * dcc, DCC_BITSTREAM_S * bs, int d)
{
	char   add_error[80];
	int    i, idx = 0;
	UDWORD tmp;
...
// return 0 on success, non-zero if error
int DccDecoder::dcc_prepare_buffer_cells(DCC_S * dcc, int d)
{
	DCC_DIRECTION_S * dir = &dcc->direction[d];
	DCC_CELL_S      * cell;
	...
	for (y = 0; y < nb_cell_h; y++)
	{
		...
		for (x = 0; x < nb_cell_w; x++)
		{
			...
			cell->w = cell_w[x];
			cell->h = cell_h[y];
			// @Toney cell->bmp = create_sub_bitmap(dir->bmp, x0, y0, cell->w, cell->h);

			cell->bmp = DsBitMapData::create_sub_bitmap(dir->bmp, x0, y0, cell->w, cell->h);
			if (cell->bmp == NULL)
			{
				sprintf(dcc_error, "dcc_prepare_buffer_cells() :\n"
...
// for stage 2
// return 0 on success, non-zero if error
int DccDecoder::dcc_prepare_frame_cells(DCC_S * dcc, int d, int f)
{
	DCC_DIRECTION_S * dir = &dcc->direction[d];
...
// return 0 on success, non-zero if error
int DccDecoder::dcc_fill_pixel_buffer(DCC_S * dcc, int d)
{
	DCC_DIRECTION_S * dir = &dcc->direction[d];
	DCC_FRAME_S     * frame;
	...
	// @Toney dir->bmp = create_bitmap(dir->box.width, dir->box.height);
	dir->bmp = DsBitMapData::create_bitmap(dir->box.width, dir->box.height);
	...
	// @Toney clear(dir->bmp); // all 0
	dir->bmp->clear_bitmap();
...
// ==========================================================================
int DccDecoder::dcc_save_frame(DCC_S * dcc, int d, int f, DsBitMapData * frm_bmp)
{
	DCC_FRAME_S * fptr = &dcc->frame[d][f];

	/// @Toney fptr->bmp = create_bitmap(frm_bmp->w, frm_bmp->h);
	fptr->bmp = DsBitMapData::clone_bitmap(frm_bmp);
	if (fptr->bmp == NULL)
	{
		/// @Toney sprintf(dcc_error, "dcc_save_frame() : can't create final frame "
		//	"bitmap of %i * %i pixels\n",
		//	frm_bmp->w, frm_bmp->h);
		return 1;
	}
	/// @Toney blit(frm_bmp, fptr->bmp, 0, 0, 0, 0, frm_bmp->w, frm_bmp->h);
...
// return 0 on success, non-zero if error
int DccDecoder::dcc_make_frames(DCC_S * dcc, int d)
{
	DCC_FRAME_S     * frame;
...
///	Sprite          * frm_bmp;
	...
	////@Toney  frm_bmp = create_bitmap(dir->box.width, dir->box.height);
	////

	frm_bmp = DsBitMapData::create_bitmap(dir->box.width, dir->box.height);
	...
	for (f = 0; f < dcc->header.frames_per_dir; f++)
	{
		////@Toney clear(frm_bmp); // clear the final frame bitmap (to index 0)
		frm_bmp->clear_bitmap();
		...
		for (c = 0; c < nb_cell; c++)
		{
			...
			if ((pbe->frame != f) || (pbe->frame_cell_index != c))
			{
				...
				{
					// different sizes
					//@Toney clear(cell->bmp); // set all pixels of the frame cell to 0
					cell->bmp->clear_bitmap();
				}
				else
				{
					// same sizes
					...
					///Sprite* sub = Sprite::createWithTexture(cell->bmp->getTexture(), Rect(cell->x0, cell->y0,
					///	cell->w, cell->h));
					///frm_bmp->addChild(sub);

					DsBitMapData::blit(dir->bmp, dir->bmp, buff_cell->last_x0, buff_cell->last_y0,
						cell->x0, cell->y0,
						cell->w, cell->h
						);
					DsBitMapData::blit(cell->bmp, frm_bmp,
						0, 0,
						cell->x0, cell->y0,
						cell->w, cell->h
						);
			...
			else
			{
				...
				if (pbe->val[0] == pbe->val[1])
				{
					...
					// @Toney clear_to_color(cell->bmp, pbe->val[0]);
					cell->bmp->clear_to_color(pbe->val[0]);
				}
				else
				{
					...
					for (y = 0; y < cell->h; y++)
					{
						for (x = 0; x < cell->w; x++)
						{
							...
							/// @Toney putpixel(cell->bmp, x, y, pbe->val[pix]);
							cell->bmp->putpixel(x, y, pbe->val[pix]);
	...
	// end
	// @Toney destroy_bitmap(frm_bmp);
	// free(frm_bmp);
	// DsBitMapData::destroy_bitmap(frm_bmp);
	frm_bmp->release();
...
// return 0 on success, non-zero if error
int DccDecoder::dcc_dir_bitstream(DCC_S * dcc, int d)
{
	DCC_BITSTREAM_S bs;
	int             f;
	char            add_error[80];
...
int DccDecoder::dcc_file_header(DCC_S * dcc)
{
	DCC_BITSTREAM_S bs;
	int             i;
...
int DccDecoder::dcc_decode(DCC_S * dcc, long dir_bitfield)
{
	int d;
...
void DccDecoder::dcc_debug(DCC_S * dcc)
{
	int d, f, i;

	log("file_signature = %i\n", dcc->header.file_signature);
	log("version        = %i\n", dcc->header.version);
	log("directions     = %i\n", dcc->header.directions);
	log("frames_per_dir = %li\n", dcc->header.frames_per_dir);
	log("tag            = %li\n", dcc->header.tag);
	log("final_dc6_size = %li bytes\n", dcc->header.final_dc6_size);
		...
		for (f = 0; f<dcc->header.frames_per_dir; f++)
		{
			...
			if (dcc->frame[d][f].optional_bytes)
			{
				...
				for (i = 0; i < (int)dcc->frame[d][f].optional_bytes; i++)
					log(" %02X", dcc->frame[d][f].optional_bytes_data[i]);
				log("\n");
			}
...
DCC_S * DccDecoder::dcc_disk_load(const char * dcc_name)
{
	DCC_S * dcc;
	FILE  * in;
	int   size;
	...
	char filename[255];
	char filename2[255];
	sprintf(filename, "D:\\Toney\\Personal\\DiabloII\\==ExtractedMPQ==\\d2data\\%s", dcc_name);
	sprintf(filename2, "D:\\Toney\\Personal\\DiabloII\\==ExtractedMPQ==\\d2char\\%s", dcc_name);
...
//    it's up to you to free the memory at the end of the program
//    with the help of dcc_destroy()
DCC_S * DccDecoder::dcc_mem_load(void * mem_ptr, int mem_size)
{
	DCC_S * dcc;
	int   size;
...
void DccDecoder::dcc_destroy(DCC_S * dcc)
{
	int d, f, nb_cell, i;
	if (dcc == NULL)
		return;
	...
	for (d = 0; d < dcc->header.directions; d++)
	{
			...
			if (dcc->frame[d][f].cell)
			{
				...
				for (i = 0; i < nb_cell; i++)
					if (dcc->frame[d][f].cell[i].bmp)
					{
						...
						dcc->frame[d][f].cell[i].bmp->release();
					}
		...
		if (dcc->direction[d].buffer_ptr)
		{
			...
			for (i = 0; i < nb_cell; i++)
				...
				{
					dcc->direction[d].buffer_ptr[i].bmp->release();
