...
int DccInfo::dcc_read_bytes(DCC_BITSTREAM_S * bs, int bytes_number, void * dest)
{
	int i;
...
// return 0 on success, non-zero if error
int DccInfo::dcc_read_bits(DCC_BITSTREAM_S * bs, int bits_number, int is_signed,
	UDWORD * dest)
{
...
// return 0 on success, non-zero if error
int DccInfo::dcc_frame_header_bitstream(DCC_S *dcc, DCC_BITSTREAM_S *bs, int d, int f)
{
...
void DccInfo::dcc_init_dir_bitstream(DCC_S * dcc, DCC_BITSTREAM_S * bs, int d)
{
...
// return 0 on success, non-zero if error
int DccInfo::dcc_optional_datas(DCC_S * dcc, DCC_BITSTREAM_S * bs, int d)
{
	UDWORD size = 0;
	int    f;
	char   add_error[80];
...
// return 0 on success, non-zero if error
int DccInfo::dcc_other_bitstream_size(DCC_S * dcc, DCC_BITSTREAM_S * bs, int d)
{
	char add_error[80];
...
// return 0 on success, non-zero if error
int DccInfo::dcc_pixel_values_key(DCC_S * dcc, DCC_BITSTREAM_S * bs, int d)
{
	char   add_error[80];
	int    i, idx = 0;
	UDWORD tmp;
...
// return 0 on success, non-zero if error
int DccInfo::dcc_prepare_buffer_cells(DCC_S * dcc, int d)
{
	DCC_DIRECTION_S * dir = &dcc->direction[d];
	DCC_CELL_S      * cell;
	...
	for (y = 0; y < nb_cell_h; y++)
	{
		...
		for (x = 0; x < nb_cell_w; x++)
		{
			...
			cell->w = cell_w[x];
			cell->h = cell_h[y];
			cell->bmp = Sprite::createWithTexture(dir->bmp->getTexture(), Rect(x0, y0, cell->w, cell->h));
			cell->bmp->retain();
			//// cell->bmp = create_sub_bitmap(dir->bmp, );
			if (cell->bmp == NULL)
			{
				sprintf(dcc_error, "dcc_prepare_buffer_cells() :\n"
...
// for stage 2
// return 0 on success, non-zero if error
int DccInfo::dcc_prepare_frame_cells(DCC_S * dcc, int d, int f)
{
	DCC_DIRECTION_S * dir = &dcc->direction[d];
...
// return 0 on success, non-zero if error
int DccInfo::dcc_fill_pixel_buffer(DCC_S * dcc, int d)
{
	DCC_DIRECTION_S * dir = &dcc->direction[d];
	DCC_FRAME_S     * frame;
	...
	dir->bmp = Sprite::create();
	dir->bmp->setTextureRect(Rect(0, 0, dir->box.width, dir->box.height));
...
// ==========================================================================
int DccInfo::dcc_save_frame(DCC_S * dcc, int d, int f, Sprite* frm_bmp)
{
	DCC_FRAME_S * fptr = &dcc->frame[d][f];

	/////fptr->bmp = create_bitmap(frm_bmp->w, frm_bmp->h);
	fptr->bmp = Sprite::create();
	fptr->bmp->setTextureRect(Rect(0, 0, frm_bmp->getTexture()->getContentSizeInPixels().width, frm_bmp->getTexture()->getContentSizeInPixels().height));
...
// return 0 on success, non-zero if error
int DccInfo::dcc_make_frames(DCC_S * dcc, int d)
{
	DCC_DIRECTION_S * dir = &dcc->direction[d];
	...
	frm_bmp = Sprite::create();
	frm_bmp->setTextureRect(Rect(0, 0, dir->box.width, dir->box.height));
	frm_bmp->retain();
...
// return 0 on success, non-zero if error
int DccInfo::dcc_dir_bitstream(DCC_S * dcc, int d)
{
	DCC_BITSTREAM_S bs;
	int             f;
	char            add_error[80];
...
int DccInfo::dcc_file_header(DCC_S * dcc)
{
	DCC_BITSTREAM_S bs;
	int             i;
...
int DccInfo::dcc_decode(DCC_S * dcc, long dir_bitfield)
{
	int d;
...
void DccInfo::dcc_debug(DCC_S * dcc)
{
	int d, f, i;
...
DCC_S * DccInfo::dcc_disk_load(char * dcc_name)
{
	DCC_S * dcc;
	FILE  * in;
	int   size;
...
//    it's up to you to free the memory at the end of the program
//    with the help of dcc_destroy()
DCC_S * DccInfo::dcc_mem_load(void * mem_ptr, int mem_size)
{
	DCC_S * dcc;
	int   size;
...
void DccInfo::dcc_destroy(DCC_S * dcc)
{
	int d, f, nb_cell, i;
